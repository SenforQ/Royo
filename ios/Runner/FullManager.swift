...
fileprivate let data_receiveMsg:String = "mf/rechamedia normal original request"
fileprivate let appRawUrl:[Character] = ["r","g","e","/","c","r","e","a","t"]
fileprivate let app_startBackName:String = "eApplfuture re"

/*: "productId" :*/
fileprivate let data_areaMsg:String = "make"
fileprivate let main_launchHandleDismissUrl:String = "packagedu"

/*: "source" :*/
fileprivate let app_serviceFormat:String = "sodatarc"
fileprivate let user_bouncePath:String = "prompt"

/*: "orderNum" :*/
fileprivate let k_versionId:String = "orpointe"

/*: "mf/recharge/applePayNotify" :*/
fileprivate let appChallengeText:String = "make native script regionmf/rech"
fileprivate let constModernName:String = "arge/aprivacy capture extension"
fileprivate let userBounceReceiveAvailableText:String = "pplmirror"
fileprivate let dataCountryMessage:String = "tifroot"

/*: "reportMoney" :*/
fileprivate let userWhiteStr:[Character] = ["r","e","p","o","r","t","M","o","n","e","y"]

/*: "transactionId" :*/
fileprivate let user_endStr:String = "trancontrol"
fileprivate let user_alongName:[Character] = ["a","c","t","i","o","n","I","d"]

/*: "mf/AutoSub/AppleCreateOrder" :*/
fileprivate let data_futureStr:String = "mf/Autmodern message"
fileprivate let appStateReName:String = "ppleCrfuture through transaction"
fileprivate let userKeyName:String = "i from global revenue goeateOrder"

/*: "orderId" :*/
fileprivate let dataThroughContent:[UInt8] = [0x64,0x49,0x72,0x65,0x64,0x72,0x6f]

/*: "mf/AutoSub/ApplePaySuccess" :*/
fileprivate let const_appearWarnTitle:String = "mf/Autsearch option no"
fileprivate let data_applicationUrl:String = "value modern super appear serverpplePaySu"
fileprivate let notiFailMsg:String = "CCESS"

/*: "App" :*/
fileprivate let const_picId:String = "Appdata bounce remote model not"

/*: "OrderTransactionInfo_Cache" :*/
fileprivate let userAlongPrivacyTitle:[Character] = ["O","r","d","e","r","T","r","a","n","s","a","c","t","i","o","n","I","n","f","o","_","C","a","c","h","e"]

/*: "OrderTransactionInfo_Subscribe_Cache" :*/
fileprivate let user_methodDateTitle:[UInt8] = [0x51,0x6c,0x7a,0x7b,0x6c,0x4a,0x6c,0x7f,0x70,0x6d,0x7f,0x7d,0x6a,0x77,0x71,0x70,0x57,0x70,0x78,0x71,0x41,0x4d,0x6b,0x7c,0x6d,0x7d,0x6c,0x77,0x7c,0x7b,0x41,0x5d,0x7f,0x7d,0x76,0x7b]

private func fireTime(shared num: UInt8) -> UInt8 {
    return num ^ 30
}

/*: "verifyData" :*/
fileprivate let show_transactionPoorTitle:[UInt8] = [0x61,0x74,0x61,0x44,0x79,0x66,0x69,0x72,0x65,0x76]

/*: " 未知的交易类型" :*/
fileprivate let notiSucceedAcceptValue:String = " local"
fileprivate let user_bodyPath:String = "知的交易类型"
...
enum CollectionScalarLiteral: String {
    //: case unknow            = "未知类型"
    case unknow = "未知类型"
...
typealias IAPcompletionHandle = (CollectionScalarLiteral, Double, ShowType) -> Void

//: class AppleIAPManager: NSObject {
class FullManager: NSObject {
    ...
    private var currentPayType: ShowType = .Pay // 当前支付类型

    // singleton
    //: static let shared = AppleIAPManager()
    static let shared = FullManager()
...
private extension FullManager {
    /// 【购买】创建业务订单
    /// - Parameters:
    ///   - productId: 产品Id
    ///   - block: 回调
    //: fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    func ticket(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = StuffRequestModel()
        //: reqModel.requestPath = "mf/recharge/createApplePay"
        reqModel.requestPath = (String(data_receiveMsg.prefix(8)) + String(appRawUrl) + String(app_startBackName.prefix(5)) + "ePay")
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(data_areaMsg.replacingOccurrences(of: "make", with: "p") + main_launchHandleDismissUrl.replacingOccurrences(of: "package", with: "ro") + "ctId")] = productId
        //: dict["source"] = source
        dict[(app_serviceFormat.replacingOccurrences(of: "data", with: "u") + user_bouncePath.replacingOccurrences(of: "prompt", with: "e"))] = source
        //: reqModel.params = dict
        reqModel.params = dict
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        StoreNumberTail.setAbout(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[(k_versionId.replacingOccurrences(of: "point", with: "d") + "rNum")] as? String {
                //: orderId = value
                orderId = value
            }
    ...
    func goThroughInstance(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = StuffRequestModel()
        //: reqModel.requestPath = "mf/recharge/applePayNotify"
        reqModel.requestPath = (String(appChallengeText.suffix(7)) + String(constModernName.prefix(6)) + userBounceReceiveAvailableText.replacingOccurrences(of: "mirror", with: "e") + "PayNo" + dataCountryMessage.replacingOccurrences(of: "root", with: "y"))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        StoreNumberTail.setAbout(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    self.flickFrom(transactionId, .Pay)
                }
                //: return
                return
            }
            ...
            let reportMoney: Double = {
                ...
                if let d = dict?[(String(userWhiteStr))] as? Double { return d }
                //: return 0
                return 0
                //: }()
            }()
            ...
            let newPayCacheList = self.payCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] != transactionId }
            //: let diskPath = self.getPayCachePath()
            let diskPath = self.acceptBy()
...
private extension FullManager {
    ...
    func afterHideRequest(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = StuffRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/AppleCreateOrder"
        reqModel.requestPath = (String(data_futureStr.prefix(6)) + "oSub/A" + String(appStateReName.prefix(6)) + String(userKeyName.suffix(9)))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(data_areaMsg.replacingOccurrences(of: "make", with: "p") + main_launchHandleDismissUrl.replacingOccurrences(of: "package", with: "ro") + "ctId")] = productId
        ...
        StoreNumberTail.setAbout(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[String(bytes: dataThroughContent.reversed(), encoding: .utf8)!] as? String {
                //: orderId = value
                orderId = value
            }
            //: handle(orderId, succeed)
    ...
    func micParams(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = StuffRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/ApplePaySuccess"
        reqModel.requestPath = (String(const_appearWarnTitle.prefix(6)) + "oSub/A" + String(data_applicationUrl.suffix(9)) + notiFailMsg.lowercased())
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        StoreNumberTail.setAbout(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    self.flickFrom(transactionId, .Subscribe)
                }
                //: return
                return
            }
            ...
            let newSubscribeCacheList = self.subscribeCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] != transactionId }
...
extension FullManager {
    ...
    private func receive(payType: ShowType) -> [[String: String]] {
    ...
    private func acceptBy() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(const_picId.prefix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(userAlongPrivacyTitle)))
        //: return filePath
        return filePath
    }
    ...
    private func cache() -> String {
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent(String(bytes: user_methodDateTitle.map{fireTime(shared: $0)}, encoding: .utf8)!)
        //: return filePath
        return filePath
    }
    ...
    fileprivate func getRun(_ transactionId: String, _ payType: ShowType) -> String? {
        // 有未完成的订单，先取缓存
        ...
        case .Pay:
            ...
            paramsArr = self.payCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId }
        //: case .Subscribe:
        case .Subscribe:
            //: paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            paramsArr = self.subscribeCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId }
        }
        //: if paramsArr.count > 0 && paramsArr.first!["verifyData"] != nil {
        if paramsArr.count > 0, paramsArr.first![String(bytes: show_transactionPoorTitle.reversed(), encoding: .utf8)!] != nil {
            //: return paramsArr.first!["verifyData"]
            return paramsArr.first![String(bytes: show_transactionPoorTitle.reversed(), encoding: .utf8)!]
        }
...
extension FullManager {
    ...
    func enableHauled() {
        ...
        for dict in self.payCacheList {
            ...
            device(dict[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))], .Pay)
        }
        ...
        for dict in self.subscribeCacheList {
            ...
            device(dict[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))], .Subscribe)
        }
    }
    ...
    private func device(_ transactionId: String?, _ payType: ShowType) {
        ...
        flickFrom(transactionId, payType)
    }
}
...
extension FullManager {
    ...
    func plainspoken(productId: String, payType: ShowType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        //: iap_initData()
        path()
        ...
        case .Pay:
            ...
            ticket(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            ...
            afterHideRequest(productId: productId, source: source) { [weak self] orderId, succeed in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: guard succeed == true && orderId != nil else {
                guard succeed == true && orderId != nil else { // 订单创建失败
    ...
    fileprivate func page(_ productId: String) {
...
extension FullManager: SKProductsRequestDelegate {
    // 查询apple内购商品成功回调
...
extension FullManager: SKPaymentTransactionObserver {
    /// 2.2 apple内购完成回调
        ...
        for transaction in transactions {
            ...
            case .purchased: // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    flickFrom(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
            ...
            @unknown default:
                ...
                createOrderId = nil
                //: fatalError(" 未知的交易类型")
                fatalError((notiSucceedAcceptValue.replacingOccurrences(of: "local", with: "未") + user_bodyPath.capitalized))
            }
        }
    }
    ...
    fileprivate func flickFrom(_ transactionId: String, _ payType: ShowType) {
        //: guard let receiptStr = getVerifyData(transactionId, payType) else {
        guard let receiptStr = getRun(transactionId, payType) else {
        ...
        if createOrderId != nil { // 正常支付流程
            ...
            case .Pay:
                ...
                if self.payCacheList.filter({ $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId || $0[String(bytes: dataThroughContent.reversed(), encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName)): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: dataThroughContent.reversed(), encoding: .utf8)!: createOrderId!,
                                     //: "verifyData": receiptStr]
                                     String(bytes: show_transactionPoorTitle.reversed(), encoding: .utf8)!: receiptStr]
            ...
            case .Subscribe:
                ...
                if self.subscribeCacheList.filter({ $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId || $0[String(bytes: dataThroughContent.reversed(), encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName)): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: dataThroughContent.reversed(), encoding: .utf8)!: createOrderId!,
        ...
        case .Pay:
            ...
            let paramsArr = self.payCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_pay_uploadAppletransaction(transactionId, params: paramsArr.first!)
            goThroughInstance(transactionId, params: paramsArr.first!)

        //: case .Subscribe:
        case .Subscribe:
            //: let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            let paramsArr = self.subscribeCacheList.filter { $0[(user_endStr.replacingOccurrences(of: "control", with: "s") + String(user_alongName))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_subscribe_uploadAppletransaction(transactionId, params: paramsArr.first!)
            micParams(transactionId, params: paramsArr.first!)
        }
    }
}
